...
CONCURRENCY = 5  # Reduced from 15 to avoid being blocked
BATCH_SIZE = 50  # Reduced batch size
CHECKPOINT_INTERVAL = 500  # Save progress more frequently
TIMEOUT = 30000  # Increased timeout to 30 seconds
MAX_RETRIES = 2  # Number of retries for failed requests
DELAY_BETWEEN_REQUESTS = (0.1, 1)  # Random delay between requests in seconds
USER_AGENTS = [
...
async def scrape_product(
    page: Page, url: str, product_index: int, total_products: int, retry_count: int = 0
) -> Optional[Dict]:
    ...
    try:
        ...
        # Essential fields - these must exist
        try:
            await page.wait_for_selector('[data-qa="modal-name"]', timeout=TIMEOUT)
            name = await page.locator('[data-qa="modal-name"]').inner_text()
            price_text = await page.locator('[data-testid="price"]').inner_text()

            if not name or not price_text or not url or not store_url:
                logging.error(f"Missing essential fields for {url}")
                return None
            ...
            logging.error(f"Failed to get essential fields for {url}: {str(e)}")
            if retry_count < MAX_RETRIES:
                logging.info(
                    f"Retrying {url} (attempt {retry_count + 1}/{MAX_RETRIES})"
                )
                return await scrape_product(
                    page, url, product_index, total_products, retry_count + 1
                )
            return None
        ...
        try:
            ...
            logging.warning(f"Could not get description for {url}")
            description = ""

        options = []
        try:
            topping_list = page.locator('[data-qa="topping-list"]')
            topping_items = await topping_list.locator('[data-qa="topping-item"]').all()

            for item in topping_items:
                try:
                    name_element = item.locator('[data-testid="typography"]').first
                    option_name = await name_element.inner_text()

                    price_elements = await item.locator(
            ...
            logging.warning(f"Could not get options for {url}")

        # Image is non-essential
        image_url = None
        try:
            ...
            logging.warning(f"Could not get image URL for {url}")
        ...
        logging.info(
            f"[{product_index}/{total_products}] Scraped product: {name} in {elapsed:.2f}s"
        )
        return product_data
...
def load_products_from_json(filepath: str) -> List[Dict]:
    ...
    Returns:
        List of product dictionaries from the JSON file
    """
    try:
...
def get_json_files(directory: str = "output") -> List[str]:
    """
    Get a list of all JSON files in the specified directory.

    Args:
        directory: Directory containing JSON files (default: 'output')
    ...
    logging.info(f"Found {len(json_files)} JSON files in {directory}")
    return json_files


def save_products(products: List[Dict], filename: Optional[str] = None):
    # Create products directory if it doesn't exist
    products_dir = "products"
    if not os.path.exists(products_dir):
        os.makedirs(products_dir)
        logging.info(f"Created directory: {products_dir}")

    if filename is None:
        filename = f"products_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"

    # Ensure the file is saved in the products directory
    filepath = os.path.join(products_dir, os.path.basename(filename))
...
def construct_product_url(store_url: str, product_id: str) -> str:
    return f"{store_url}?productDetail={product_id}"


def get_checkpoint_filename() -> str:
    # Create products directory if it doesn't exist
    products_dir = "products"
    if not os.path.exists(products_dir):
        os.makedirs(products_dir)
    ...
    return os.path.join(products_dir, checkpoint_filename)


async def create_browser_context(
    browser: Browser, cookies: List[Dict] = None
) -> BrowserContext:
    """Create a browser context with settings to avoid detection"""
...
async def process_batch(
    ...
    try:
        # Create browser contexts and pages for this batch
        for _ in range(min(CONCURRENCY, len(urls))):
            context = await create_browser_context(browser, cookies)
        ...
        for i, url in enumerate(urls):
            ...
            task = asyncio.create_task(
                scrape_product(page, url, start_index + i, total_urls)
            )
            tasks.append(task)
        ...
        all_results = results_so_far + valid_results
        if (start_index + len(urls)) % CHECKPOINT_INTERVAL == 0 and all_results:
            checkpoint_file = get_checkpoint_filename()
            save_products(all_results, checkpoint_file)
...
async def main():
    ...
    if not json_files:
        logging.error("No JSON files found to process")
        return
    ...
    async with async_playwright() as p:
        ...
        try:
            for file_index, json_file in enumerate(json_files):
                logging.info(
                    f"Processing file {file_index + 1}/{total_files}: {os.path.basename(json_file)}"
                )

                # Load products from this file
                products_data = load_products_from_json(json_file)
                if not products_data:
                    logging.warning(f"No products found in {json_file}, skipping")
                    continue

                logging.info(
                    f"Loaded {len(products_data)} products from {os.path.basename(json_file)}"
                )

                # Create URLs to scrape
                urls_to_scrape = [
                    construct_product_url(product["store_url"], product["product_id"])
                ...
                logging.info(
                    f"Starting to scrape {total_urls} unique products from {os.path.basename(json_file)}..."
                )
                ...
                for i in range(0, total_urls, BATCH_SIZE):
                    ...
                    logging.info(
                        f"Processing batch {i//BATCH_SIZE + 1}/{(total_urls+BATCH_SIZE-1)//BATCH_SIZE} ({len(batch_urls)} products)"
                    )

                    batch_results = await process_batch(
                        browser, batch_urls, i, total_urls, file_results, cookies
                    )
                    file_results.extend(batch_results)

                    logging.info(
                        f"Batch complete. {len(batch_results)}/{len(batch_urls)} successful. Total so far: {len(file_results)}/{i+len(batch_urls)}"
                    )

                # Save results for this file
                if file_results:
                    output_filename = f"scraped_{os.path.basename(json_file)}"
                    save_products(file_results, output_filename)
                    all_results.extend(file_results)
                else:
                    logging.warning(
                        f"No products were successfully scraped from {os.path.basename(json_file)}!"
                    )

                file_end_time = time.time()
                file_elapsed = file_end_time - file_start_time
                logging.info(
                    f"Completed scraping {os.path.basename(json_file)} in {file_elapsed:.2f} seconds. "
                    f"Successfully scraped {len(file_results)} products out of {total_urls} attempts."
                )

                # Add a delay between files
                if file_index < total_files - 1:
                    file_delay = random.uniform(5, 10)
                    logging.info(
                        f"Waiting {file_delay:.2f}s before processing next file..."
                    )
                    await asyncio.sleep(file_delay)

            # Save combined results
            if all_results:
                save_products(all_results, "all_products.json")

            end_time = time.time()
            elapsed = end_time - start_time
            logging.info(
                f"Completed scraping all files in {elapsed:.2f} seconds. "
                f"Successfully scraped {len(all_results)} products in total."
            )
